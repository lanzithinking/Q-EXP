...
if __name__ == '__main__':
    ...
    # test
    linv.test(1e-8, MF_only=False)
    # obtain MAP
    map_v = linv.get_MAP(SAVE=True, NCG=True)#,init_opt='LSE',lmda=.1)
    print('MAP estimate: '+(min(len(map_v),10)*"%.4f ") % tuple(map_v[:min(len(map_v),10)]) )
    #  compare it with the truth
    true_param = linv.misfit.truth
    map_f = linv.prior.vec2fun(map_v) if linv.prior.space=='vec' else map_v
    ...
    # min_cost = linv._get_misfit(map_v)
