"""
Main function to run whitened (geometric) dimension-independent sampling for linear inverse problem
----------------------
Shiwei Lan @ ASU, 2022
----------------------
"""
...
def main(seed=2022):
    ...
    try:
        ...
        #     map=pickle.load(f)
        # f.close()
        # z_init=linv.whiten.qep2wn(map).flatten(order='F')
        u_init=linv.init_parameter if hasattr(linv,'init_parameter') else linv._init_param()#init_opt='LSE',lmda=.1)
        z_init=linv.whiten.qep2wn(u_init).flatten(order='F')
    except Exception as e:
        print(e)
        z_init=linv.whiten.sample()
    h=1e-7; v=np.random.randn(linv.prior.dim)
    l,g=linv.get_geom(z_init,geom_ord=[0,1],whitened=True)[:2]; hess=linv.get_geom(z_init,geom_ord=[1.5],whitened=True)[2]
    Hv=hess(v)
    l1,g1=linv.get_geom(z_init+h*v,geom_ord=[0,1],whitened=True)[:2]
    ...
    winfMC=wht_geoinfMC(z_init,linv,args.step_sizes[args.alg_NO],args.step_nums[args.alg_NO],args.algs[args.alg_NO],transformation=linv.whiten.wn2qep, MF_only=True, whitened=True, k=100)
    ...
    # filename='linv_'+args.algs[args.alg_NO]+'_dim'+str(len(u))+'_'+prior_params['prior_option']+'_'+prior_params['ker_opt']+'_'+ctime+'.pckl'
    ...
    try:
        ...
        med_f = np.median(samp,axis=0).reshape(linv.misfit.size)
        mean_f = np.mean(samp,axis=0).reshape(linv.misfit.size)
        std_f = np.std(samp,axis=0).reshape(linv.misfit.size)
    except Exception as e:
        print(e)
        mean_f=0; std_f=0
        n_samp=samp.shape[0]
        for i in range(n_samp):
            samp_i=linv.prior.vec2fun(samp[i]) if linv.prior.space=='vec' else samp[i]
            mean_f+=samp_i/n_samp
            std_f+=samp_i**2/n_samp
        std_f=np.sqrt(std_f-mean_f**2)
        mean_f=mean_f.reshape(linv.misfit.size)
        std_f=std_f.reshape(linv.misfit.size)
        med_f=None
    if med_f is not None:
        linv.misfit.plot_data(img=med_f, save_img=True, save_path='./MCMC/'+prior_params['prior_option'], save_fname=args.algs[args.alg_NO]+'_median')
    linv.misfit.plot_data(img=mean_f, save_img=True, save_path='./MCMC/'+prior_params['prior_option'], save_fname=args.algs[args.alg_NO]+'_mean')
    linv.misfit.plot_data(img=std_f, save_img=True, save_path='./MCMC/'+prior_params['prior_option'], save_fname=args.algs[args.alg_NO]+'_std')
