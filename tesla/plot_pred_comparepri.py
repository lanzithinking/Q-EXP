...
# obtain estimates
folder = './MAP_hldout'
if os.path.exists(os.path.join(folder,'Tesla_'+str(len(tr_idx))+'days_prediction_summary.pckl')):
    f=open(os.path.join(folder,'Tesla_'+str(len(tr_idx))+'days_prediction_summary.pckl'),'rb')
    dat,maps,pred_m,pred_s=pickle.load(f)
    f.close()
    print('Tesla_'+str(len(tr_idx))+'days_prediction_summary.pckl has been read!')
else:
    ...
    for m in range(num_mdls):
        ...
        # prior_params['sigma2'] = 1000 if prior_params['prior_option']=='gp' else sigma2
        ts = Tesla(**prior_params,**lik_params,seed=seed)
        dat = ts.misfit.obs
        ...
        for f_i in pckl_files:
            if '_'+str(len(tr_idx))+'days_' in f_i and '_'+pri_mdls[m]+'_' in f_i:
                try:
                    ...
                    pass
    # save
    f=open(os.path.join(folder,'Tesla_'+str(len(tr_idx))+'days_prediction_summary.pckl'),'wb')
...
for i,ax in enumerate(axes.flat):
    ...
    ax.plot([ts.misfit.times[tr_idx]]*2,[np.zeros(len(tr_idx))+90,np.ones(len(tr_idx))*10+90], color='black', linewidth=.5)
...
# save plot
# fig.tight_layout()
plt.savefig(os.path.join(folder,'Tesla_'+str(len(tr_idx))+'days_preds_comparepri.png'),bbox_inches='tight')
# plt.show()
