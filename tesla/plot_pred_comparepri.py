...
# obtain estimates
folder = './MAP_hldout'
if os.path.exists(os.path.join(folder,'Tesla_'+ts.misfit.truth_name+'_prediction_summary.pckl')):
    f=open(os.path.join(folder,'Tesla_'+ts.misfit.truth_name+'_prediction_summary.pckl'),'rb')
    truth,maps,pred_m,pred_s=pickle.load(f)
    f.close()
    print('Tesla_'+ts.misfit.truth_name+'_prediction_summary.pckl has been read!')
else:
    ...
    maps=np.stack(maps)
    # save
    f=open(os.path.join(folder,'Tesla_'+ts.misfit.truth_name+'_prediction_summary.pckl'),'wb')
...
if os.path.exists(os.path.join('.','Tesla_'+ts.misfit.truth_name+'_prediction_summary.pckl')):
    f=open(os.path.join('.','Tesla_'+ts.misfit.truth_name+'_prediction_summary.pckl'),'rb')
    truth,maps,pred_m,pred_s=pickle.load(f)
    f.close()
    print('Tesla_'+ts.misfit.truth_name+'_prediction_summary.pckl has been read!')
num_mdls=3 
...
# save plot
# fig.tight_layout()
plt.savefig(os.path.join(folder,'Tesla_'+ts.misfit.truth_name+'_preds_comparepri.png'),bbox_inches='tight')
# plt.show()
