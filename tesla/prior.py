#!/usr/bin/env python
"""
Class definition of prior for the time series model.
--------------------------------------------------------------------------
Created February 15, 2022 for project of Spatiotemporal Besov prior (STBP)
updated October 4, 2022 for project of q-exponential process prior (Q-EXP)
"""
...
class _gp(GP):
    """
    Gaussian process prior GP(mu,C) defined on 1d domain D.
    """
    def __init__(self,input,L=None,mean=None,store_eig=False,**kwargs):
        x=self._grid(gdsz=input) if np.size(input)==1 else input
        self.space=kwargs.pop('space','fun') # alternative 'vec'
    ...
    def _grid(self,gdsz=None):
        ...
        self.gdsz=gdsz
        # set the grid
        grid=np.linspace(0,2,self.gdsz)
        # print('\nThe grid is defined.')
        return grid
...
class _bsv(BSV):
    """
    Besov prior measure B(mu,_C) defined on 1d domain D.
    """
    def __init__(self,input,L=None,mean=None,store_eig=False,**kwargs):
        x=self._grid(gdsz=input) if np.size(input)==1 else input
        self.space=kwargs.pop('space','fun') # alternative 'vec'
...
class _qep(qEP):
    """
    q-exponential process prior q-EP(mu,C) defined on 1d domain D.
    """
    def __init__(self,input,L=None,mean=None,store_eig=False,**kwargs):
        x=self._grid(gdsz=input) if np.size(input)==1 else input
        self.space=kwargs.pop('space','fun') # alternative 'vec'
...
if __name__ == '__main__':
    ...
    input=128
    store_eig=True
    prior = prior(prior_option=prior_option, ker_opt='covf', cov_opt='matern', input=input, basis_opt='Fourier', q=1., s=1, L=100, store_eig=store_eig, space='fun')
    # generate sample
    u=prior.sample()
