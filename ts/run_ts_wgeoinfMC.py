"""
Main function to run whitened (geometric) dimension-independent sampling for the time series problem
----------------------
Shiwei Lan @ ASU, 2022
----------------------
"""
...
def main(seed=2022):
    ...
    l,g=ts.get_geom(z_init,geom_ord=[0,1],whitened=True)[:2]; hess=ts.get_geom(z_init,geom_ord=[1.5],whitened=True)[2]
    Hv=hess(v)
    ...
    winfMC=wht_geoinfMC(z_init,ts,args.step_sizes[args.alg_NO],args.step_nums[args.alg_NO],args.algs[args.alg_NO],transformation=ts.whiten.wn2qep, MF_only=True, whitened=True, k=100)
    ...
    # filename='ts_'+ts.misfit.truth_name+'_'+args.algs[args.alg_NO]+'_dim'+str(len(u))+'_'+prior_params['prior_option']+'_'+prior_params['ker_opt']+'_'+ctime+'.pckl'
