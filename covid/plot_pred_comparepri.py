...
truth_option = 2
size = 229 if truth_option==0 else 100 if truth_option==1 else 50
ker_opt = 'covf'
cov_opt = 'matern'
basis_opt = 'Fourier'
KL_trunc = 100
...
#ts = Covid(truth_option=truth_option, cov_opt=cov_opt, basis_opt=basis_opt, KL_trunc=KL_trunc, space=space, sigma2=sigma2, s=s, store_eig=store_eig, seed=seed)
 # train index
tr_idx = np.hstack([np.arange(int(ts.misfit.size/2),dtype=int),int(ts.misfit.size/2)+np.arange(0,int(ts.misfit.size/8*4),2,dtype=int)])
...
# obtain estimates
folder = './MAP_hldout'
if os.path.exists(os.path.join(folder,'Covid_'+ts.misfit.truth_name+'_prediction_summary.pckl')):
    f=open(os.path.join(folder,'Covid_'+ts.misfit.truth_name+'_prediction_summary.pckl'),'rb')
    truth,maps,pred_m,pred_s=pickle.load(f)
    f.close()
    print('Covid_'+ts.misfit.truth_name+'_prediction_summary.pckl has been read!')
else:
    ...
    maps=np.stack(maps)
    # save
    f=open(os.path.join(folder,'Covid_'+ts.misfit.truth_name+'_prediction_summary.pckl'),'wb')
...
if os.path.exists(os.path.join('.','Covid_'+ts.misfit.truth_name+'_prediction_summary.pckl')):
    f=open(os.path.join('.','Covid_'+ts.misfit.truth_name+'_prediction_summary.pckl'),'rb')
    truth,maps,pred_m,pred_s=pickle.load(f)
    f.close()
    print('Covid_'+ts.misfit.truth_name+'_prediction_summary.pckl has been read!')
num_mdls=3 
...
num_rows=1
# posterior median
fig,axes = plt.subplots(nrows=num_rows,ncols=num_mdls,sharex=True,sharey=True,figsize=(16,5)) #(12,5)
titles = mdl_names
...
# save plot
# fig.tight_layout()
plt.savefig(os.path.join(folder,'Covid_'+ts.misfit.truth_name+'_preds_comparepri.png'),bbox_inches='tight')
# plt.show()
