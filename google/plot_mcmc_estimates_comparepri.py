...
# obtain estimates
folder = './analysis'
if os.path.exists(os.path.join(folder,'Google_'+str(ts.misfit.size)+'days_mcmc_summary.pckl')):
    f=open(os.path.join(folder,'Google_'+str(ts.misfit.size)+'days_mcmc_summary.pckl'),'rb')
    dat,med_f,mean_f,std_f=pickle.load(f)
    f.close()
    print('Google_'+str(ts.misfit.size)+'days_mcmc_summary.pckl has been read!')
else:
    med_f=[[]]*num_mdls
    mean_f=[[]]*num_mdls
    std_f=[[]]*num_mdls
        ...
        if os.path.exists(fld_m):
            ...
            for f_i in npz_files:
                try:
                    ...
                    pass
    # save
    f=open(os.path.join(folder,'Google_'+str(ts.misfit.size)+'days_mcmc_summary.pckl'),'wb')
...
# save plot
# fig.tight_layout()
plt.savefig(folder+'/Google_'+str(ts.misfit.size)+'days_mcmc_estimates_comparepri.png',bbox_inches='tight')
# plt.show()
