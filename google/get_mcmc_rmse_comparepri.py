...
# obtain estimates
folder = './analysis'
if not os.path.exists(os.path.join(folder,'Google_'+str(ts.misfit.size)+'days_mcmc_summary.pckl')):
    med_f=[[]]*num_mdls
    mean_f=[[]]*num_mdls
    std_f=[[]]*num_mdls
...
rmse_m.to_csv(os.path.join(folder,'Google_'+str(ts.misfit.size)+'days-RMSE-mean.csv'),columns=mdl_names[:num_mdls])
rmse_s.to_csv(os.path.join(folder,'Google_'+str(ts.misfit.size)+'days-RMSE-std.csv'),columns=mdl_names[:num_mdls])