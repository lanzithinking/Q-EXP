#!/usr/bin/env python
"""
Class definition of linear inverse problem of Shepp-Logan head phantom.
Shiwei Lan @ ASU 2022
----------------------------------------------------------------------------
Created February 15, 2022 for project of q-exponential process prior (Q-EXP)
Updated April 26, 2023 for project of q-exponential process prior (Q-EXP)
"""
...
class CT:
    ...
    def setup(self, seed=2022, **kwargs):
        ...
        # set prior
        self.prior = prior(input=self.misfit.truth if kwargs.get('ker_opt','serexp')=='graphL' else self.misfit.size,L=self.KL_trunc,**kwargs)
    ...
    def _init_param(self,init_opt='LSE',**kwargs):
        """
        ...
        if hasattr(self.misfit, reconstruction_method):
            ...
            self.init_parameter = reconstruct(**kwargs).flatten(order='F')
        else:
            self.init_parameter = self.misfit.truth.flatten(order='F')
...
if __name__ == '__main__':
    ...
    # test
    ct.test(1e-8, MF_only=False)
    # obtain MAP
    map_v = ct.get_MAP(SAVE=True, NCG=False)#,init_opt='LSE',lmda=.1)
    print('MAP estimate: '+(min(len(map_v),10)*"%.4f ") % tuple(map_v[:min(len(map_v),10)]) )
    #  compare it with the truth
    true_param = ct.misfit.truth
    map_f = ct.prior.vec2fun(map_v) if ct.prior.space=='vec' else map_v
    map_f = map_f.reshape(true_param.shape, order='F')
    ...
    # min_cost = ct._get_misfit(map_v)
    # print('Minimum cost: %.4f' % min_cost)
    # plot MAP
    import matplotlib.pyplot as plt
    fig=ct.misfit.plot_data(img=map_f, save_img=True, save_path='./properties', save_fname='truth_map')
    plt.show()