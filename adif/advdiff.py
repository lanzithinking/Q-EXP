...
class advdiff(TimeDependentAD,SpaceTimePointwiseStateObservation):
    ...
    def setup(self,seed=2020,**kwargs):
        ...
        self.prior = _prior(Vh=self.pde.Vh[PARAMETER], gamma=self.gamma, delta=self.delta, **kwargs)
...
#     mesh = dl.Mesh('ad_10k.xml')
    ...
    plt.colorbar(fig)
#     plt.show()
    plt.savefig(os.path.join(os.getcwd(),'properties/map_'+{'gp':'GP','bsv':'Besov','qep':'qEP'}[prior_option]+'.png'),bbox_inches='tight')
